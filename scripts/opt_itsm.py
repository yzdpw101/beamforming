"""ITSM 稀布面阵优化 — 108元, 对称, PSLL(φ=0)+PSLL(φ=90) 最小化"""
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

import numpy as np, time
from antopt.mapping import ITSM
...
mapper = ITSM(Ne=Ne, Lx=Lx, Ly=Ly, dmin=dmin, is_symmetric=True)
print(f"ITSM: Nq={mapper.Nq}  grid={mapper.max_rows}x{mapper.max_cols}  vars={mapper.n_vars}")
...
def fitness(x):
    ...
    af_x = pat.linear_af_symmetric(fx)
...
result = minimize(
    ...
    sigma=0.5, pop_size=False, max_iter=5000,
    n_jobs=-1, verbose=True, seed=42,
)
elapsed = time.perf_counter() - t0

# ── 结果 ──
fx, fy = mapper.synthesize(result["x"], expand_to_4q=False)
psll0 = _psll_of(fx)
psll90 = _psll_of(fy)

fx_full, fy_full = mapper.synthesize(result["x"], expand_to_4q=True)
dist = np.sqrt((fx_full[:,None]-fx_full[None,:])**2 + (fy_full[:,None]-fy_full[None,:])**2)
...
GX, GY = np.meshgrid(np.arange(x0, x1+dmin/2, dmin), np.arange(y0, y1+dmin/2, dmin))
...
ax.set_title(f"ITSM 第一象限 {mapper.Nq}元  dmin={dmin}λ"); ax.set_aspect('equal')

for ax, pos, label in [(axes[1], fx, "phi=0"), (axes[2], fy, "phi=90")]:
    af_db = Pattern.to_dB(pat.linear_af_symmetric(pos))
...
fig.suptitle(f"ITSM稀布面阵 {Ne}元  Lx={Lx}λ Ly={Ly}λ  PSLL和={psll0+psll90:.1f}dB", fontsize=13)
fig.tight_layout()
fig.savefig("itsm_opt.png", dpi=150); plt.close(fig)
print("图片已保存: itsm_opt.png")
